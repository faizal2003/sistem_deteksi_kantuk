...
class DrowsinessDetectionApp:
    def __init__(self, root):
        ...
        self.head_tilt_label.grid(row=0, column=2, padx=5, pady=1, sticky="w")
        
        self.mar_label = tk.Label(self.status_frame, text="MAR: 0.00", 
                                 font=status_font, bg="#8DA9DB", fg="green", width=12, anchor="w")
        self.mar_label.grid(row=0, column=3, padx=5, pady=1, sticky="w")
        
        # NEW: Add eye tracking labels - third row
        self.left_eye_pos_label = tk.Label(self.status_frame, text="Left Eye: Center", 
                                          font=status_font, bg="#8DA9DB", fg="purple", width=12, anchor="w")
        self.left_eye_pos_label.grid(row=1, column=0, padx=5, pady=1, sticky="w")
        
        self.right_eye_pos_label = tk.Label(self.status_frame, text="Right Eye: Center", 
                                           font=status_font, bg="#8DA9DB", fg="purple", width=12, anchor="w")
        self.right_eye_pos_label.grid(row=1, column=1, padx=5, pady=1, sticky="w")
        
        # NEW: Add eye ratio labels - fourth row
        self.left_eye_ratio_label = tk.Label(self.status_frame, text="L-Ratio: 0.00", 
                                            font=status_font, bg="#8DA9DB", fg="blue", width=12, anchor="w")
        self.left_eye_ratio_label.grid(row=1, column=2, padx=5, pady=1, sticky="w")
        
        self.right_eye_ratio_label = tk.Label(self.status_frame, text="R-Ratio: 0.00", 
                                             font=status_font, bg="#8DA9DB", fg="blue", width=12, anchor="w")
        self.right_eye_ratio_label.grid(row=1, column=3, padx=5, pady=1, sticky="w")
    ...
    def stop_detection(self):
        ...
        # NEW: Reset eye tracking labels
        self.left_eye_pos_label.config(text="Left Eye: Center")
        self.right_eye_pos_label.config(text="Right Eye: Center")
        self.left_eye_ratio_label.config(text="L-Ratio: 0.00")
        self.right_eye_ratio_label.config(text="R-Ratio: 0.00")
    ...
    # NEW: Helper method to convert eye position to text direction
    def position_to_direction(self, pos):
        """Convert position coordinates to direction text"""
        if pos is None:
            return "Unknown"
        ...
        while self.is_running:
            ...
            # Normal operation mode after calibration
                ...
                for face in faces:
                    ...
                    # NEW: Initialize eye tracking variables
                    leftRatio = rightRatio = 0
                    leftPos = rightPos = None
                    leftColor = rightColor = None
                    
                    try:
                        # Get eye tracking data from module if available
                        image, PointList = m.faceLandmakDetector(frame, gray_frame, face, False)
                        
                        # Get left and right eye points from the module's landmark detector
                        # Note: The module's order might be reversed compared to dlib's order
                        LeftEyePoint = PointList[36:42]  # Left eye in the module
                        RightEyePoint = PointList[42:48]  # Right eye in the module
                        
                        # Get blink detection data
                        leftRatio, lTop, lBottom = m.blinkDetector(LeftEyePoint)
                        rightRatio, rTop, rBottom = m.blinkDetector(RightEyePoint)
                        
                        # Get eye tracking data
                        maskRight, rightPos, rightColor = m.EyeTracking(frame, gray_frame, RightEyePoint)
                        maskLeft, leftPos, leftColor = m.EyeTracking(frame, gray_frame, LeftEyePoint)
                        
                        # Convert positions to readable directions
                        left_eye_direction = self.position_to_direction(leftPos)
                        right_eye_direction = self.position_to_direction(rightPos)
                        
                        # Update eye tracking labels
                        self.root.after(0, lambda dir=left_eye_direction: self.left_eye_pos_label.config(
                            text=f"Left Eye: {dir}"))
                        self.root.after(0, lambda dir=right_eye_direction: self.right_eye_pos_label.config(
                            text=f"Right Eye: {dir}"))
                        
                        # Update eye ratio labels
                        self.root.after(0, lambda ratio=leftRatio: self.left_eye_ratio_label.config(
                            text=f"L-Ratio: {ratio:.2f}"))
                        self.root.after(0, lambda ratio=rightRatio: self.right_eye_ratio_label.config(
                            text=f"R-Ratio: {ratio:.2f}"))
                        
                        # Optionally, visualize eye tracking on the display frame
                        if leftPos is not None:
                            # Draw left eye gaze point
                            eye_center = np.mean(LeftEyePoint, axis=0).astype(int)
                            # Draw a circle at the center of the left eye
                            cv2.circle(display_frame, tuple(eye_center), 3, (0, 255, 255), -1)
                            # Draw a line indicating gaze direction
                            gaze_end = (eye_center[0] + int(leftPos[0]*10), eye_center[1] + int(leftPos[1]*10))
                            cv2.line(display_frame, tuple(eye_center), gaze_end, (255, 0, 0), 2)
                        
                        if rightPos is not None:
                            # Draw right eye gaze point
                            eye_center = np.mean(RightEyePoint, axis=0).astype(int)
                            # Draw a circle at the center of the right eye
                            cv2.circle(display_frame, tuple(eye_center), 3, (0, 255, 255), -1)
                            # Draw a line indicating gaze direction
                            gaze_end = (eye_center[0] + int(rightPos[0]*10), eye_center[1] + int(rightPos[1]*10))
                            cv2.line(display_frame, tuple(eye_center), gaze_end, (255, 0, 0), 2)
            ...
            # If calibrated but no face detected
                ...
                # Reset eye tracking labels when no face is detected
                self.root.after(0, lambda: self.left_eye_pos_label.config(text="Left Eye: --"))
                self.root.after(0, lambda: self.right_eye_pos_label.config(text="Right Eye: --"))
                self.root.after(0, lambda: self.left_eye_ratio_label.config(text="L-Ratio: --"))
                self.root.after(0, lambda: self.right_eye_ratio_label.config(text="R-Ratio: --"))
